...
class Programacion extends \yii\db\ActiveRecord
{
    ...
    public function getProgramacionSemanal($data)
    {
        ...
        $result =$command->createCommand("SELECT *, (Cantidad / PiezasMolde) AS Moldes, CASE WHEN Cantidad >= PTB THEN PTB ELSE 0 END AS FaltaPTB, (Cantidad - (CTB + PLB +PMB)) AS FaltaCast FROM f_GetProgramaciones($area,$params) Where Estatus = 'Abierto' ORDER BY Producto, FechaEmbarque ASC ")->queryAll();
